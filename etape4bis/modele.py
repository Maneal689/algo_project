...
class Case:
    ...
    def select(self):
        """Retourne la valeur de select, qui indique si la bille est 'sélectionnée' ou non
        Case -> Boolean"""
        return (self.__select)

    def change_select(self, val):
        """Change la sélection de la case
        Case -> None"""
        self.__select = val
...
class ModeleSame:
    ...
    def select(self, i, j):
        """Vérifie si la case en position i, j est sélectionnée ou non
        ModeleSame, int, int -> Boolean"""
        return (self.__mat[i][j].select())

    def change_select(self, i, j, val):
        """
        Change la sélection de la Case en position i, j
        ModeleSame, int, int -> None
        """
        l_billes_selectionnees = []
        l_billes_selectionnees.append((i, j))
        self.ajoute_meme_couleur(l_billes_selectionnees, i, j)
        for coord in l_billes_selectionnees:
            self.__mat[coord[0]][coord[1]].change_select(val)
    ...
    def supprime_bille(self,i,j):
        ...
        if (self.__mat[i][j].couleur() != -1):
            l_asuppr = []
            l_asuppr.append((i, j))
            self.ajoute_meme_couleur(l_asuppr, i, j)
            if (len(l_asuppr) >= 2):
                self.__score += (len(l_asuppr) - 2) ** 2
                for elm in l_asuppr:
                    self.__mat[elm[0]][elm[1]].change_select(False)
    ...
    def ajoute_meme_couleur(self, lsuppr, i, j):
        """
        ...
        for coord in coord_at:
            if (self.__mat[coord[0]][coord[1]].couleur() == couleur and coord not in lsuppr):
                ...
                self.ajoute_meme_couleur(lsuppr, coord[0], coord[1])
    ...
    def tombe_bille(self):
        """
        Fait tomber les billes à la verticale
        ModeleSame -> None
        """
        i = 0
